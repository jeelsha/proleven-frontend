...
export const AddEditManagerModal = ({
  ...
  const [clientGetApi, { isLoading: getCompaniesDataLoading }] = useAxiosGet();
  const [paginateCompanyData] = useAxiosGet();
  ...
  const getCompany = async () => {
    const response = await clientGetApi(`/companies`, {
      params: { dropdown: true, label: 'name', role: role?.toString() },
    });
    if (response?.data) {
  ...
  const loadOptions = async (page: number, debounceSearch?: string) => {
    const obj = {
      page,
      search: debounceSearch,
      role: role?.toString()
    }
    const response = await paginateCompanyData(`/companies`, {
      params: { ...obj },
    });

    if (response?.data) {
      return response?.data?.data?.map((option: { name: string, id: string }) => ({
        label: option.name,
        value: option.id,
      }));
    }
