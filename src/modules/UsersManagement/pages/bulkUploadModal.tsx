...
const BulkUploadModal = ({
  ...
  const extractTextFromHyperlink = (value: unknown): unknown => {
  ...
  const newFunction = (
    ...
    const cleanValue = extractTextFromHyperlink(value);

    let finalValue = cleanValue;
    if (columnKey === 'mobile_number') {
      finalValue =
        cleanValue !== undefined && cleanValue !== null ? String(cleanValue) : '';
    }
    ...
    if (columnKey?.includes('.')) {
      ...
      result = createNestedObject(columnKey, finalValue, result);
      temp[columnKey.split('.')[0]] = result[columnKey.split('.')[0]];
    } else {
      temp[columnKey] = finalValue;
    }
  };
      ...
      if (worksheet) {
        ...
        if (rowData) {
          rowData.forEach((data: any, rowIndex: number) => {
            if (data && typeof data === 'object' && data.richText) {
              // Concatenate text from richText objects
              let combinedText = '';
              data.richText.forEach((item: any) => {
                combinedText += item.text || ''; // Concatenate text values
              });
              excelStructure[Number(rowIndex)] = combinedText;
            } else if (typeof data === 'string') {
              // Handle non-richText data
              excelStructure[Number(rowIndex)] = data;
            }
          });

          const dropDownOpts: string[] = [];

          rowData.forEach((data: any) => {
            if (data && typeof data === 'object' && data.richText) {
              // Extract text from richText objects for dropdown options
              let combinedText = '';
              data.richText.forEach((item: any) => {
                if (item.text) {
              ...
              if (combinedText && dropDownOpts.indexOf(combinedText) === -1) {
                dropDownOpts.push(combinedText);
              }
            } else if (
              typeof data === 'string' &&
              dropDownOpts.indexOf(data) === -1
            ) {
              dropDownOpts.push(data);
            }
          });

          // Remove existing values from dropdown options if they match any form field's value
          Array.from(formFields.keys()).forEach((field) => {
            const formField = formFields.get(field);
            if (formField?.value) {
              const index = dropDownOpts.indexOf(formField.value);
              if (index !== -1) {
          ...
          if (Number(Object.values(excelStructure).length) < checkLength) {
            isSuccess = false;
            dispatch(
              setToast({
                variant: 'Error',
                            ...
                            <ReactSelect
                              ...
                              onChange={(value) => {
                                ...
                                if (
                                  ...
                                  const filteredNotEmptyOptions = temp.filter(
                                    (item) => item !== ''
                                  );
                                  setDropdownOptions(
                                    filteredNotEmptyOptions.filter(
