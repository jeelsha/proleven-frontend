...
import BundleInvitation from '../CourseInvitation/bundleInvitation';
...
const TrainerDashboard = () => {
  ...
  const { response: todayCourse, isLoading: isTrainerCourseLoading } =
    useQueryGetFunction(`/dashboard/trainer/today/${user?.username}`);

  const { response: trainerTotalRating } = useQueryGetFunction(
    `/dashboard/trainer-rate`
  );
  ...
  return (
    <>
      <Tiles
        trainerDashboardData={trainerDashboardData}
        user={user}
        totalRating={
          trainerTotalRating?.data
            ? Number(trainerTotalRating?.data).toFixed(2)
            : undefined
        }
      />
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-7 mb-7 last:mb-0">
        <CourseInvitation dashboard />
        <BundleInvitation dashboard />
        <Course
          title={t('Dashboard.Trainer.Course.title')}
          courses={todayCourse?.data?.data}
          isCourseLoading={isTrainerCourseLoading}
        />
        {state !== null && state.isFirstTimeLogin === true && (
          <Modal
            hideCloseIcon
            closeOnEscape={false}
