...
// ** Course Types **
import { CourseInitialValues, CourseSubComponentProps } from 'modules/Courses/types';
      ...
      <>
        <div className="grid grid-cols-2 gap-3">
          {values?.exam?.questions?.[ind]?.answers?.map((item, index) => {
            return (
              <div className="flex gap-3" key={`Answer_${index + 1}`}>
                <InputField
    ...
    return (
      <>
        {(values?.exam?.questions ?? []).map((quest, ind) => {
          return (
            <div
              className="flex border-b border-solid border-borderColor pb-5 last:pb-0 last:border-none"
              key={`Question_${ind + 1}`}
            >
