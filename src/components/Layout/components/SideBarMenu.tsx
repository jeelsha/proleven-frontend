...
const SideBarMenu = ({ menuData, activeData, hide, setActiveSubMenu }: IMenu) => {
  ...
  const isActive =
    ...
    menuData.subRoute?.some(
      (item) => item.uniqueId === activeData || activeData === menuData.uniqueId
    );
  ...
  const commonClasses = `
      text-sm cursor-pointer flex items-center transition-all duration-500 group
      ${
        isActive
      ...
      ${
        openSidebar
          ? 'w-full rounded-lg md:rounded-r-none py-2 md:py-3 ps-4 md:ps-8 md:pe-2.5'
          : 'w-fit p-2.5 rounded-lg md:rounded-r-none ms-2.5'
      }
    `;
      ...
      <>
        <Button className="inline-block w-5 h-5 text-current">
          <Image
            ...
            iconClassName="stroke-current w-full h-full flex-shrink-0"
          />
        </Button>
        {openSidebar ? (
          <Button className="text-current block overflow-hidden whitespace-nowrap max-w-[calc(100%_-_24px)] ps-3.5">
            {menuData.label}
          </Button>
        ) : (
          ''
  ...
  return (
    <li className="group/main">
      {menuData.subRoute ? (
        <Button className={commonClasses} onClickHandler={handleClick}>
          {renderChild()}
        </Button>
      ) : (
        <Link
            ...
            {menuData.subRoute?.map((item) => (
              <li key={item?.id} className="">
                <Link
                  ...
                  className={`text-left block font-medium text-sm leading-4 relative transition-all duration-300 pr-2
                  ${
                    openSidebar
